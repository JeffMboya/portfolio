...
const MOCK_PROPELLER_MDX = `---
title: Propeller
slug: propeller
description: A distributed WebAssembly task execution platform.
stack:
  - Go
  - Rust
date: "2024-03"
...
stack:
  - TypeScript
date: "2023-06"
github: https://github.com/JeffMboya/other
...
describe('lib/projects', () => {
  beforeEach(async () => {
    ...
    vi.mocked(fs.readdirSync).mockReturnValue(['propeller.mdx', 'other.mdx'] as any)
    vi.mocked(fs.readFileSync).mockImplementation((filePath: any) => {
      if (String(filePath).includes('propeller')) return MOCK_PROPELLER_MDX
      if (String(filePath).includes('other')) return MOCK_OTHER_MDX
      throw new Error(`ENOENT: ${filePath}`)
    })
    vi.mocked(fs.existsSync).mockImplementation((filePath: any) => {
      return String(filePath).includes('propeller') || String(filePath).includes('other')
    })
  })
  ...
  describe('getAllProjects', () => {
    it('returns only featured projects', async () => {
      ...
      expect(projects[0].slug).toBe('propeller')
    })

    it('sorts by date descending', async () => {
      const fs = await import('fs')
      const OLDER_MDX = MOCK_PROPELLER_MDX.replace('2024-03', '2023-01')
      vi.mocked(fs.readFileSync).mockImplementation((filePath: any) => {
        if (String(filePath).includes('propeller')) return OLDER_MDX
...
---
content`
      vi.mocked(fs.readdirSync).mockReturnValue(['propeller.mdx', 'newer.mdx'] as any)
      vi.mocked(fs.readFileSync).mockImplementation((filePath: any) => {
  ...
  describe('getProjectBySlug', () => {
    it('returns meta and content for a valid slug', async () => {
      ...
      const { meta, content } = getProjectBySlug('propeller')
      expect(meta.title).toBe('Propeller')
      expect(meta.stack).toContain('Go')
  ...
  describe('getAllProjectSlugs', () => {
    it('returns slugs derived from filenames', async () => {
      ...
      expect(slugs).toContain('propeller')
      expect(slugs).toContain('other')
      expect(slugs).not.toContain('propeller.mdx')
    })
  })
})

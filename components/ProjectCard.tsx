  ...
  return (
    <Link
      ...
      className="block border border-[#e8e8e8] rounded-xl p-7 bg-white
        hover:border-[#ccc] hover:translate-y-[-2px]
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-1.5 mt-3.5">
            {meta.stack.map((s) => (
              <span
                key={s}
                className="bg-[#f5f5f5] text-[#666] text-[11px] px-2 py-0.5 rounded"
              >
                {s}
              </span>
            ))}
        ...
        <span className="text-[#bbb] text-lg mt-1 group-hover:text-[#888] transition-colors duration-150">
          ↗
        </span>
      </div>
    </Link>
